//*#########################################################
//* ANCHOR -draw Waypoints
//*#########################################################

export function drawWaypoints(ctx, waypoints, waypoint_color) {
  ctx.strokeStyle = waypoint_color;
  ctx.lineWidth = 20;
  ctx.beginPath();
  waypoints.forEach((waypoint, index) => {
    const adjustedY = waypoint.y + 40; // Verschiebe die y-Koordinate um 50 Pixel nach unten
    if (index === 0) {
      ctx.moveTo(waypoint.x + 10, adjustedY);
    } else {
      ctx.lineTo(waypoint.x + 10, adjustedY);
    }
    ctx.arc(waypoint.x + 10, adjustedY, 2, 0, Math.PI * 2);
  });
  ctx.stroke();
}
const levels = [
  {
    //* ANCHOR - Level: Grassland
    name: "Grassland",
    background_img_path: "src/assets/bg/bg2.webp",
    waypoint_color: "rgba(241, 207, 113, 0.9)",
    waypoints: [
      { x: -50, y: 20 },
      { x: 50, y: 20 },
      { x: 340, y: 20 },
      { x: 340, y: 110 },
      { x: 30, y: 110 },
      { x: 30, y: 180 },
      { x: 350, y: 180 },
      { x: 350, y: 260 },
      { x: 30, y: 260 },
      { x: 30, y: 340 },
      { x: 450, y: 340 },
    ],
    tower_places: [
      {
        x: 360,
        y: 365,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 70,
        y: 15,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 295,
        y: 15,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 90,
        y: 165,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 250,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 130,
        y: 330,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 350,
        y: 330,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 330,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 180,
        y: 330,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 280,
        y: 165,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 90,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 195,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 210,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Desert:
    name: "Desert",
    background_img_path: "src/assets/bg/desert_bg.png",
    waypoint_color: "rgba(231, 137, 55, 0.9)",
    waypoints: [
      { x: -50, y: 80 },
      { x: 50, y: 80 },
      { x: 110, y: 80 },
      { x: 110, y: 0 },
      { x: 270, y: 0 },
      { x: 270, y: 110 },
      { x: 140, y: 110 },
      { x: 140, y: 190 },
      { x: 360, y: 190 },
      { x: 360, y: 250 },
      { x: 110, y: 250 },
      { x: 110, y: 330 },
      { x: 450, y: 330 },
    ],
    tower_places: [
      {
        x: 330,
        y: 355,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 60,
        y: 15,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 295,
        y: 15,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 10,
        y: 185,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 50,
        y: 345,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 130,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 350,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 180,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 200,
        y: 175,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 82,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 165,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 210,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Snow:
    name: "Snow",
    background_img_path: "src/assets/bg/bg_snow.png",
    waypoint_color: "rgba(255, 255, 255, 0.9)",
    waypoints: [
      { x: -50, y: 65 },
      { x: 50, y: 65 },
      { x: 110, y: 65 },
      { x: 110, y: 0 },
      { x: 270, y: 0 },
      { x: 270, y: 110 },
      { x: 140, y: 110 },
      { x: 140, y: 170 },
      { x: 270, y: 170 },
      { x: 270, y: 230 },
      { x: 110, y: 230 },
      { x: 110, y: 330 },
      { x: 450, y: 330 },
    ],
    tower_places: [
      {
        x: 345,
        y: 352,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 5,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 150,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 10,
        y: 185,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 50,
        y: 345,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 130,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 350,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 180,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 185,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 82,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 85,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 50,
        y: 270,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 210,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Swamp:
    name: "Swamp",
    background_img_path: "src/assets/bg/bg_swamp.png",
    waypoint_color: "rgba(62, 39, 3, 0.9)",
    waypoints: [
      { x: -50, y: 20 },
      { x: 20, y: 20 },
      { x: 20, y: 150 },
      { x: 130, y: 150 },
      { x: 130, y: 20 },
      { x: 130, y: 20 },
      { x: 200, y: 20 },
      { x: 200, y: 250 },
      { x: 20, y: 250 },
      { x: 20, y: 335 },
      { x: 240, y: 335 },
      { x: 240, y: 70 },
      { x: 360, y: 70 },
      { x: 360, y: 335 },
      { x: 450, y: 335 },
    ],
    tower_places: [
      {
        x: 345,
        y: 355,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 60,
        y: 15,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 40,
        y: 130,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 95,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 10,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 85,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 130,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 290,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 280,
        y: 340,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 180,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 82,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 140,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 240,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Apokalypse:
    name: "Apokalypse",
    background_img_path: "src/assets/bg/bg_apo.png",
    waypoint_color: "rgba(186, 138, 59, 0.9)",
    waypoints: [
      { x: -50, y: 120 },
      { x: 70, y: 120 },
      { x: 150, y: 60 },
      { x: 330, y: 130 },
      { x: 30, y: 270 },
      { x: 30, y: 310 },
      { x: 310, y: 310 },
      { x: 310, y: 290 },
      { x: 365, y: 250 },
      { x: 365, y: 180 },
      { x: 300, y: 140 },
      { x: 30, y: 270 },
      { x: 30, y: 310 },
      { x: 450, y: 310 },
    ],
    tower_places: [
      {
        x: 345,
        y: 330,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 18,
        y: 80,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 40,
        y: 180,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 5,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 10,
        y: 250,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 85,
        y: 300,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 130,
        y: 300,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 275,
        y: 37,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 280,
        y: 300,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 180,
        y: 300,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 82,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 220,
        y: 240,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 190,
        y: 65,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 100,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 200,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Grasland 2:
    name: "Grasland2",
    background_img_path: "src/assets/bg/bg_grasland2.png",
    waypoint_color: "transparent",
    waypoints: [
      { x: -50, y: 120 },
      { x: 125, y: 120 },
      { x: 110, y: 50 },
      { x: 150, y: -10 },
      { x: 240, y: -10 },
      { x: 200, y: 40 },
      { x: 220, y: 90 },
      { x: 140, y: 160 },
      { x: 190, y: 210 },
      { x: 230, y: 260 },
      { x: 190, y: 300 },
      { x: 200, y: 340 },
      { x: 190, y: 300 },
      { x: 230, y: 260 },
      { x: 300, y: 280 },
      { x: 450, y: 290 },
    ],
    tower_places: [
      {
        x: 345,
        y: 310,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 18,
        y: 110,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 148,
        y: 87,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 85,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 90,
        y: 355,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 23,
        y: 267,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 130,
        y: 280,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 310,
        y: 55,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 244,
        y: 320,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 295,
        y: 350,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 244,
        y: 90,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 82,
        y: 200,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 300,
        y: 213,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 201,
        y: 170,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 100,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 266,
        y: 17,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - FreeBuild (neu)
    name: "FreeBuild",
    background_img_path: "src/assets/bg/pure_grass.png",
    waypoint_color: "rgba(200,200,200,0.2)",
    free_build: true,
    // Start / End points used by pathfinder
    spawn_start: { x: -50, y: 20 },
    spawn_end: { x: 450, y: 340 },
    waypoints: [],
    tower_places: [],
  },
  {
    //* ANCHOR - Lavaland:
    name: "Lavaland",
    game_mode: "ctf",
    background_img_path: "src/assets/bg/bg_vulcano.png",
    waypoint_color: "rgba(10, 10, 10, 0.12)",
    tower_place_fill: "rgba(255, 255, 255, 0.18)",
    tower_place_stroke: "rgba(255, 255, 255, 0.55)",
    waypoints: [
      { x: 50, y: -60 },
      { x: 170, y: 30 },
      { x: 240, y: 120 },
      { x: 123, y: 190 },
      { x: 230, y: 270 },
      { x: 124, y: 348 },
    ],
    tower_places: [
      {
        x: 190,
        y: 326,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 103,
        y: 64,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 250,
        y: 37,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 75,
        y: 127,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 10,
        y: 175,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 50,
        y: 355,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 80,
        y: 220,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 294,
        y: 286,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 270,
        y: 350,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 300,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 260,
        y: 145,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 329,
        y: 93,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 340,
        y: 184,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 337,
        y: 240,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 237,
        y: 90,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 228,
        y: 197,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Swamp2:
    name: "Swamp2",
    background_img_path: "src/assets/bg/swamp2.jpg",
    waypoint_color: "rgba(62, 39, 3, 0.5)",
    waypoints: [
      { x: -50, y: -20 },
      { x: 30, y: -10 },
      { x: 90, y: 60 },
      { x: 220, y: 20 },
      { x: 266, y: 87 },
      { x: 270, y: 112 },
      { x: 363, y: 145 },
      { x: 363, y: 170 },
      { x: 270, y: 225 },
      { x: 120, y: 137 },
      { x: 60, y: 200 },
      { x: 100, y: 210 },
      { x: 155, y: 283 },
      { x: 70, y: 340 },
      { x: 390, y: 340 },
      // { x: 450, y: 335 },
    ],
    tower_places: [
      {
        x: 345,
        y: 355,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 208,
        y: 147,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 40,
        y: 130,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 110,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 10,
        y: 200,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 3,
        y: 312,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 338,
        y: 245,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 290,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 247,
        y: 187,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 88,
        y: 118,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 150,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 117,
        y: 205,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 229,
        y: 295,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 164,
        y: 259,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 360,
        y: 20,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 240,
        y: 10,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
  {
    //* ANCHOR - Ice2:
    name: "Ice2",
    background_img_path: "src/assets/bg/ice2.1.jpg",
    waypoint_color: "rgba(229, 244, 244, 0.3)",
    waypoints: [
      { x: -40, y: 80 },
      { x: 40, y: 60 },
      { x: 110, y: 20 },
      { x: 130, y: -20 },
      { x: 200, y: -20 },
      { x: 70, y: 80 },
      { x: 190, y: 115 },
      { x: 20, y: 180 },
      { x: 10, y: 230 },
      { x: 190, y: 300 },
      { x: 30, y: 400 },
    ],
    tower_places: [
      {
        x: 125,
        y: 354,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 40,
        cooldown: 0,
        is_trap: true,
      },
      {
        x: 220,
        y: 130,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 85,
        y: 328,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 140,
        y: 93,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 5,
        y: 154,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 3,
        y: 312,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 296,
        y: 307,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 290,
        y: 60,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 247,
        y: 187,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 194,
        y: 210,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: -5,
        y: 47,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 356,
        y: 135,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 231,
        y: 270,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 170,
        y: 250,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 54,
        y: 143,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
      {
        x: 27,
        y: 361,
        tower_is_build: false,
        tower_damage_lvl: 1,
        tower_type: "",
        tower_img: "",
        range: 80,
        cooldown: 0,
      },
    ],
  },
];

export function set_level(lvl_id) {
  let level_details = undefined;
  if (lvl_id === "level_rnd") {
    level_details = random_level();
  } else {
    level_details = levels[lvl_id];
  }

  return level_details;
}

//* Random Map
function random_level() {
  let rnd_index = Math.floor(Math.random() * levels.length);
  // rnd_index = 9;
  return levels[rnd_index];
}
